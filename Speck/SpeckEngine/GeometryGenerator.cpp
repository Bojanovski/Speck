...
GeometryGenerator::StaticMeshData GeometryGenerator::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount)
{
	...
	// Add bottom vertices
	int bottomIndexStart = (int)meshData.Vertices.size();
	for (uint32 j = 0; j <= sliceCount; ++j)
	{
		float theta = (2.0f*j + 1.0f)*0.5f*thetaStep; // must be in the middle of the slice
	...
	{
		meshData.Indices32.push_back(topIndexStart + i);
	...
	baseIndex = topIndexStart - ringVertexCount;
	for(uint32 i = 0; i < sliceCount; ++i)
	{
		meshData.Indices32.push_back(bottomIndexStart + i);
